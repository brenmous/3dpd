...
module master_front() {
    difference() { 
        import ("/home/bren/3dpd/kp3s_mods/orbiter_mount_brian/ez_prnt_print_head_front.stl");
...
module master() {
    difference() { 
        import ("/home/bren/3dpd/kp3s_mods/orbiter_mount_brian/ez_prnt_print_head_back_with_blt.stl");
...
module mount_extension() {
    ...
    difference() {
        ...
        for (i=[0:20]) {
            color("red")
            translate([-113.8+i,55.2+i,-3.588])
            linear_extrude(height=3.5-0.022)
            union() {
                offset(r=1)
                square(size=[20,18]);
        ...
        color("red")
        translate([-113.8+39,55.5+18.5,-3.588])
        cube([5,5,5]);
    }
...
// a whole bunch of messy cuts to remove weight
module cut() {
    difference() {
        difference() {
            new_back();
            translate([-48.9 - mount_yoffset + 10, 34, -5])
            color("red")
            cube([50,20.9,15]);
            ...
            color("black")
            translate([-34.99, 54.4, -6])
            cube([3.99, 8, 10]);

            //color("green")
            //translate([-34.99, 54.4, 2.5])
            //cube([5.5, 7.7, 50]);

            color("red")
            translate([-32, 61.4, 10])
            cube([5.5, 1, 10]);

            color("red")
            translate([-40, 60.9, 6.345])
            cube([20, 1.5, 10]);
        }
        color("green")
        translate([-34.99, 54.4, 2.5])
...
module filled() {
    ...
    intersection() {
        cut();
        shift=2.2;
        translate([-55 - shift,30 + shift,-4.6])
        rotate([0,0,-44])
        color("green")
        cube([5, 36, 7.1]);
    }
